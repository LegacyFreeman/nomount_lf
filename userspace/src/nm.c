...
void c_main(long *sp) {
    ...
    fd = syscall(SYS_SOCKET, AF_NETLINK, SOCK_RAW, NETLINK_GENERIC);
    ...
    if (cmd == 'a' || cmd == 'd') {
        ...
        for (int arg_idx = 2; arg_idx + step <= argc; arg_idx += step) {
            char *v_end = resolve_path(v_resolved, cwd, argv[arg_idx]);
            int v_len = v_end - v_resolved;
            if (!v_len) { exit_code = 3; continue; }

            int r_len = 0;
            if (is_add) {
                char *r_end = resolve_path(r_resolved, cwd, argv[arg_idx+1]);
            ...
            if (is_add) {
                *(unsigned int*)cursor = 0; cursor += 4;
                *(unsigned short*)cursor = (unsigned short)v_len; cursor += 2;
                *(unsigned short*)cursor = (unsigned short)r_len; cursor += 2;
        ...
        do_nm_cmd(fd, nm_family, NOMOUNT_CMD_CLEAR_ALL, 0, (void *)0, 0, NLM_F_REQUEST | NLM_F_ACK);

    } else if (cmd == 'v') {
        if (do_nm_cmd(fd, nm_family, NOMOUNT_CMD_GET_VERSION, 0, (void *)0, 0, NLM_F_REQUEST | NLM_F_ACK) > 0) {
        ...
        unsigned int len = do_nm_cmd(fd, nm_family, NOMOUNT_CMD_GET_LIST, 0, (void *)0, 0, NLM_F_REQUEST | NLM_F_DUMP);
